...
def assume_role_credentials(mfa_profile, role_profile):
    ...
    roleArn = 'arn:aws:iam::AWS_ACCOUNT_ID:role/THEROLE'
    roleArn = roleArn.replace('AWS_ACCOUNT_ID', rba_ak['account_id'])
    roleArn = roleArn.replace('THEROLE', rba_ak['role'])
...
def generate_magic_link(profile, role_profile):
    ...
    roleArn = 'arn:aws:iam::AWS_ACCOUNT_ID:role/THEROLE'
    roleArn = roleArn.replace('AWS_ACCOUNT_ID', rba['account_id'])
    roleArn = roleArn.replace('THEROLE', rba['role'])
...
if __name__ == '__main__':
    if len(sys.argv) > 2:
        ...
        print("Use: ./aws-rba-wrapper.py rba_profile command")
        print("Example: ./aws-rba-wrapper.py admin@prod aws sts get-caller-identity")
        print("Example: ./aws-rba-wrapper.py admin@prod mc")
        exit(1)

    if "AWS_RBA_WRAPPER_USER" not in os.environ:
        profile = input("AWS_RBA_WRAPPER_USER not set, please enter the profile: ")
    else:
        profile = os.environ['AWS_RBA_WRAPPER_USER']
